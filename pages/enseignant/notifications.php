...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['mark_notification_read'])) {
    ...
    if ($notificationId) {
        try {
            $stmt = $connexion->prepare("UPDATE Notification SET status = 'read' WHERE id_notification = :id_notification AND id_enseignant = :id_enseignant");
            $stmt->execute([':id_notification' => $notificationId, ':id_enseignant' => $_SESSION['id']]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['mark_all_read'])) {
    try {
        $stmt = $connexion->prepare("UPDATE Notification SET status = 'read' WHERE id_enseignant = :id_enseignant AND status = 'unread'");
...
try {
    ...
    $stmt = $connexion->prepare("
        ...
        WHERE n.id_enseignant = :id_enseignant
